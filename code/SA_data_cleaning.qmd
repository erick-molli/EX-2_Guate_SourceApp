...
Now I will create a variable called `group`, which will be coded as 'Baseline', 'Control' or 'Intervention'.
...
And now I will rename some of the variables and delete unnecessary ones. I will also recode the values from the newly named `arm` variable, and set `date` in date format.
...
Both the personal exposure sample and the blank dataframes are ready to be used for further analysis, so I saved them in .CSV format.

```{r}
write_csv(hapin_filters, file = here("data", "processed-data", "hapin_samples.csv"))
write_csv(hapin_blanks, file = here("data", "processed-data", "hapin_blanks.csv"))
```
...
Some observations contain brackets, which represent values below the detection limit and they have to be corrected in future steps. So first, here I will delete the brackets, then I will convert these variables to numeric type and round them to 4 decimal digits.
...
bdl <- concentration %>%
  summarise(
    ...
    percent_bdl_cl = sum(Cl < 0.003) / n() * 100,
    percent_bdl_na = sum(Na < 0.04) / n() * 100)
...
bdl2 <- concentration %>%
  summarise(
    count_bdl_mg = sum(Mg < 0.0051),
    count_bdl_al = sum(Al < 0.0059),
    count_bdl_si = sum(Si < 0.0068),
    count_bdl_s = sum(S < 0.0026),
    count_bdl_k = sum(K < 0.0040),
    count_bdl_ca = sum(Ca < 0.0040),
    count_bdl_ti = sum(Ti < 0.0086),
    count_bdl_cr = sum(Cr < 0.0055),
    count_bdl_mn = sum(Mn < 0.0030),
    count_bdl_fe = sum(Fe < 0.0043),
    count_bdl_ni = sum(Ni < 0.0043),
    count_bdl_cu = sum(Cu < 0.0080),
    count_bdl_zn = sum(Zn < 0.0024),
    count_bdl_ga = sum(Ga < 0.0049),
    count_bdl_as = sum(As < 0.0062),
    count_bdl_se = sum(Se < 0.0181),
    count_bdl_cd = sum(Cd < 0.0932),
    count_bdl_in = sum(In < 0.1089),
    count_bdl_sn = sum(Sn < 0.1844),
    count_bdl_te = sum(Te < 0.2166),
    count_bdl_i = sum(I < 0.3149),
    count_bdl_pb = sum(Pb < 0.0324),
    count_bdl_cl = sum(Cl < 0.003),
    count_bdl_na = sum(Na < 0.04))

print(bdl2)
```

Only 12 elements have less than 50% of their observations below the detection limit. But first, I will process the concentrations for all chemical species (including the ones with more than 50% of their observations BDL), so they are included into the appendix table. First, blank adjusting all the values, by substracting the median value for each element, obtained from the blank filters. Also, replace all values below the detection limit with (LOD/sqrt(2)).

```{r}
#Substract the median blank value for the elements that need blank adjustment
concentration <- concentration %>%
  mutate(Mg = Mg - 0.0112, S = S - 0.0044, K = K - 0.0014, Ca = Ca - 0.0048, Fe = Fe - 0.0038, Cu = Cu - 0.0043, Zn = Zn - 0.0045, Sn = Sn - 0.0050, Te = Te - 0.020, I = I - 0.10, Na = Na - 0.0135)
...
concentration <- concentration %>% 
    mutate(Mg = round(if_else(Mg < 0.0051, 0.0051 / sqrt(2), Mg), 4), Al = round(if_else(Al < 0.0059, 0.0059 / sqrt(2), Al), 4),
           ...
           Ti = round(if_else(Ti < 0.0086, 0.0086 / sqrt(2), Ti), 4), Cr = round(if_else(Cr < 0.0055, 0.0055 / sqrt(2), Cr), 4),
           Mn = round(if_else(Mn < 0.0030, 0.0030 / sqrt(2), Mn), 4), Fe = round(if_else(Fe < 0.0043, 0.0043 / sqrt(2), Fe), 4), 
          Ni = round(if_else(Ni < 0.0043, 0.0043 / sqrt(2), Ni), 4), Cu = round(if_else(Cu < 0.0080, 0.0080 / sqrt(2), Cu), 4),  
          Zn = round(if_else(Zn < 0.0024, 0.0024 / sqrt(2), Zn), 4), Ga = round(if_else(Ga < 0.0049, 0.0049 / sqrt(2), Ga), 4),
          As = round(if_else(As < 0.0062, 0.0062 / sqrt(2), As), 4), Se = round(if_else(Se < 0.0181, 0.0181 / sqrt(2), Se), 4),  
          Cd = round(if_else(Cd < 0.0932, 0.0932 / sqrt(2), Cd), 4), In = round(if_else(In < 0.1089, 0.1089 / sqrt(2), In), 4),  
          Sn = round(if_else(Sn < 0.1844, 0.1844 / sqrt(2), Sn), 4), Te = round(if_else(Te < 0.2166, 0.2166 / sqrt(2), Te), 4),  
          I = round(if_else(I < 0.3149, 0.3149 / sqrt(2), I), 4), Pb = round(if_else(Pb < 0.0324, 0.0324 / sqrt(2), Pb), 4),  
           Cl = round(if_else(Cl < 0.003, 0.003 / sqrt(2), Cl), 4), Na = round(if_else(Na < 0.04, 0.04 / sqrt(2), Na), 4))
```

I will save these concentrations in a new dataframe `concentrations_ugcm2` that will be used for uncertainty estimations in future steps. First, removing all unnecessary species (Species with 50% \> B LoD), then arranging the observations based on the LoD.

```{r}
#Select only the filter ID, type and the 12 elements that have more than 50% of their data not below the detection limit
concentrations_ugcm2 <- concentration %>% select(c(filter_id, Mg, Al, Si, S, K, Ca, Ti, Mn, Fe, Zn, Cl, Na)) %>% 
  arrange(filter_id) #Also I decided to arrange them ascending based on the filter_id
```

And then, converting the concentrations from micrograms per square centimeter to micrograms per cubic meter using the equation: ug/m3 = (Concentration (ug/cm^2^) x Filter area (cm^2^))/Sample volume (m3). Where, the filter area is 0.7853 cm^2^ and the sample volume is 0.43 m^3^. And then saving this data frame to `concentration_full`, since this will be used to compute the summary statistics of all original elemental species.

```{r}
#Converting the concentrations from (ug/cm2) to (ug/m3)
concentration <- concentration %>%
  mutate(across(Mg:Na, ~ round(.x * 0.7853 / 0.43, 4))) %>% 
  arrange(filter_id) #Also I decided to arrange them ascending based on the filter_id
```

Then, pasting the BC and PM2.5 concentrations from the `hapin_filters` dataframe.
...
concentration <- bind_cols(concentration, hapin_filters$bc) %>% rename(BC = "...27") %>% select(-Type) #Join the BC concentrations to the `concentrations` df, also renaming the column to "BC" and deleting the 'Type' column
concentration <- bind_cols(concentration, hapin_filters$pm25) %>% rename(PM25 = "...27")
```

I will add the fueltype, arm and group variables to this dataframe.

```{r}
#Add the `arm`, `fueltype` and `group` variables to the `concentration` dataframe
concentration <- concentration %>%
  mutate(arm = hapin_filters$arm, fueltype = hapin_filters$fueltype, group = hapin_filters$group)
```

Then, saving this dataframe to `concentrations_full`, to compute the summary statistics of all the chemical species.

```{r}
write_csv(concentration, here("data", "processed-data", "concentrations_full.csv"))
```

Now, I will filter the elements with less than 50% of their observations below detection limit.

```{r}
#Select only the filter ID, type and the 12 elements that have more than 50% of their data not below the detection limit
concentration <- concentration %>% select(c(filter_id, arm, fueltype, group, Mg, Al, Si, S, K, Ca, Ti, Mn, Fe, Zn, Cl, Na, BC, PM25))
```
...
```{r}
uncertainty <- uncertainty %>% select(c(Filter, Mg, Al, Si, S, K, Ca, Ti, Mn, Fe, Zn, Cl, Na)) %>% #Select only the necessary variables
  rename(filter_id = "Filter") %>% #Rename the Filter column
  mutate_at(vars(Mg:Na), ~ round(., 4)) %>% #Round the uncertainties to 4 decimal places
...
variables <- c('Mg', 'Al', 'Si', 'S', 'K', 'Ca', 'Ti', 'Mn', 'Fe', 'Zn', 'Cl', 'Na')
...
Then, estimate the BC uncertainties using an equation also derived from the law of propagation of uncertainties.

```{r}
#First I will estimate the BC uncertainties
uncertainty <- uncertainty %>% 
  mutate(BC = round((1370 * sqrt((atn$ATN)^2 * 0.000005^2 + 0.000079^2 * 1.16^2)), 4))
```

Now, I will estimate the uncertainties for the PM2.5 concentrations. First I calculate the raw mass as micrograms and then apply the equation derived from the law of propagation of uncertainties.

```{r}
#Estimate raw PM2.5 in micrograms
uncertainty <- uncertainty %>%
  mutate(pm25raw = round((0.43 * concentration$PM25), 4))

#Calculate PM2.5 uncertainties
uncertainty <- uncertainty %>% 
  mutate(PM25 = round((sqrt(((1/0.43)^2 *(2)^2) + (-(pm25raw)/(0.43)^2 )^2 * (0.05 * 0.43)^2)), 4))
```

I will remove the `pm25raw` variable and I will also add the `arm`, `fueltype` and `group` variables to this dataframe, and I'll save it as .CSV format.

```{r}
#First I will remove the pm25raw variable
uncertainty <- uncertainty %>% select(-pm25raw)

#Add the `arm`, `fueltype` and `group` variables to the `uncertainty` dataframe
uncertainty <- uncertainty %>%
  mutate(arm = hapin_filters$arm, fueltype = hapin_filters$fueltype, group = hapin_filters$group)
```
...
gg_miss_var(uncertainty)
```

There is one observation with missing PM2.5 and BC concentrations and uncertainties. After doing some research, there is no record that the data information from this filter is available, so the best decision was to drop this value. And then, save the df to a .CSV file.
...
write_csv(concentration, file = here("data", "processed-data", "concentration.csv"))
write_csv(uncertainty, file = here("data", "processed-data", "uncertainty.csv"))
```

Finally, the files need to be saved for the PMF software, so I will remove unnecessary variables

```{r}
#Revmove arm, fueltype and group variables
concentration_final <- concentration %>% select(-c(arm, fueltype, group))
uncertainty_final <- uncertainty %>% select(-c(arm, fueltype, group))

#Save files
write_csv(concentration_final, file = here("data", "processed-data", "concentration_final.csv"))
write_csv(uncertainty_final, file = here("data", "processed-data", "uncertainty_final.csv"))
```

DONE! :)

PART BELOW IS PENDING, ONLY IF AMBIENT FILTERS WILL BE ANALYZED (NOT THE CASE FOR NOW)

## Ambient filters processing

### Ambient filters concentrations

First, loading the ambient filter concentrations.

```{r}
#Loading the ambient filters chemical species concentration data
ambient <- read_excel(here("data", "raw-data", "ambient_conc_unc_hapingt.xlsx"), sheet = "Concentration")
```

First, deleting the brackets, then I will convert these variables to numeric type and round them to 5 decimal digits.

```{r}
#Remove brackets, convert to numeric, and round to 4 decimal places
ambient <- ambient %>%
  mutate(across(2:25, ~ round(as.numeric(str_remove_all(., "\\[|\\]")), 5))) %>% 
  rename(filter_id = "Filter")
```
...
Only 6 elements have less than 50% of their observations below the detection limit. Now, I will filter the remaining elements (variables).

```{r}
#Select only the filter ID, type and the 10 elements that have more than 50% of their data not below the detection limit
ambient <- ambient %>% select(c(filter_id, Mg, S, K, Ca, Fe, Zn)) %>% 
  arrange(filter_id) #Also I decided to arrange them ascending based on the filter_id
```

Replace all values below the detection limit with (LOD/sqrt(2)).

```{r}
#Replace all values below the detection limit with DL/sqrt(2)
ambient <- ambient %>% 
    mutate(Mg = round(if_else(Mg < 0.0051, 0.0051 / sqrt(2), Mg), 4), S = round(if_else(S < 0.0026, 0.0026 / sqrt(2), S), 4),
           K = round(if_else(K < 0.0016, 0.0016 / sqrt(2), K), 4), Ca = round(if_else(Ca < 0.0040, 0.0040 / sqrt(2), Ca), 4),
...
amb_conc_ugcm2 <- ambient
```

And then, converting the concentrations from micrograms per square centimeter to micrograms per cubic meter using the equation: ug/m3 = (Concentration (ug/cm^2^) x Filter area (cm^2^))/Sample volume (m3). Where, the filter area is 12.56 cm^2^ and the sample volume is 2.75 m^3^.

```{r}
#Converting the concentrations from (ug/cm2) to (ug/m3)
ambient <- ambient %>%
  mutate(across(Mg:Zn, ~ round(.x * 12.56 / 2.75, 4)))
```

If possible, I will add the BC concentrations (they have to be estimated apparently). Then, save the dataframe in .CSV format

### Ambient filters uncertainties

First, loading the ambient filter concentrations.

```{r}
#Loading the ambient filters chemical species uncertainties data
ambient_unc <- read_excel(here("data", "raw-data", "ambient_conc_unc_hapingt.xlsx"), sheet = "Uncertainty")
```

First, I will remove the chemical species with more than 50% of their values below detection limit, sort by Filter ID and round the values to numbers with five decimal places.

```{r}
ambient_unc <- ambient_unc %>% select(c(Filter, Mg, S, K, Ca, Fe, Zn)) %>% #Select only the necessary variables
  rename(filter_id = "Filter") %>% #Rename the Filter column
  mutate_at(vars(Mg:Zn), ~ round(., 5)) %>% #Round the uncertainties to 4 decimal places
  arrange(filter_id) #Arrange observations based on Filter ID
```
...
```{r}
#Create a list for the detection limits for each element
amb_dl <- c(Mg = 0.0051, S = 0.0026, K = 0.0016, Ca = 0.0040, Fe = 0.0043, Zn = 0.0024)

# Recalculate the values in df based on the conditions
ambient_unc <- ambient_unc %>%
  mutate(across(all_of(names(amb_dl)),
                ~ round(if_else(amb_conc_ugcm2[[cur_column()]] < amb_dl[[cur_column()]], (5/6) * amb_dl[[cur_column()]], .), 4),
                .names = "{.col}"))
```
...
```{r}
# Define the variables to be recalculated
ambient_variables <- c('Mg', 'S', 'K', 'Ca', 'Fe', 'Zn')

# Recalculate the values using the given equation
ambient_unc <- ambient_unc %>%
  mutate(across(all_of(ambient_variables), ~ round( #Here, selecting all the variables to be re-calculated
    sqrt((12.56/2.75)^2 * .^2 + (amb_conc_ugcm2[[cur_column()]]/2.75)^2 * 0.05^2 + (-amb_conc_ugcm2[[cur_column()]]*12.56/2.75^2)^2 * (0.05 * 2.75)^2), 4), .names = "{.col}")) #Input the equation
```

If possible, I will add the BC uncertainties, based on the BC concentrations that will apparently have to be estimated. Then, save the dataframe in .CSV format.
